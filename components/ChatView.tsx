...
import { SendIcon, PixelBotIcon, BotAvatar, MenuIcon } from './Icons';
...
const ChatView: React.FC<ChatViewProps> = ({ 
    ...
    return (
        <div className="flex flex-col h-full bg-white relative">
            {/* Header with dynamic padding to account for floating toggle */}
            <header className={`h-20 border-b border-gray-100 flex items-center bg-white/90 backdrop-blur-md z-[50] sticky top-0 transition-all ${isSidebarOpen ? 'px-8' : 'pl-24 pr-8'}`}>
                ...
                <div className="w-full max-w-3xl mx-auto pointer-events-auto">
                    <div className="bg-white rounded-[2.5rem] shadow-[0_20px_50px_rgba(106,91,255,0.12)] flex items-center p-3 border border-indigo-50/50">
                        <input
                            ...
                            className="flex-1 bg-transparent border-none text-base text-indigo-900 placeholder-indigo-300 focus:outline-none focus:ring-0 px-8 font-bold"
                            disabled={isLoading}
                        />
                        <button
                            onClick={handleSend}
